...
class TS_transformer(torch.nn.Module):
    ...
    def __init__(self, args):
        ...
        # encoders
        self.spatial_encoder_1 = TransformerModel(self.emsize, self.nhead, self.nhid, self.nlayers, self.dropout)
        self.spatial_encoder_2 = TransformerModel(self.emsize, self.nhead, self.nhid, self.nlayers, self.dropout)
        ...
        self.input_embedding_layer_temporal = nn.Linear(2, self.emsize)
        self.input_embedding_layer_spatial = nn.Linear(2, self.emsize)

        # Linear layer to output and fusion
        self.noise_layer = nn.Linear(self.emsize + self.noise_size, self.emsize)
    ...
    def forward(self, inputs):
        ...
        outputs = torch.zeros(self.args.obs_length, nodes_abs.shape[1], self.emsize).cuda()
        GM = torch.zeros(self.args.obs_length, nodes_abs.shape[1], self.emsize).cuda()

        for framenum in range(self.args.obs_length):

            nei_list = nei_lists[framenum, :, :]
            nodes_current = nodes_norm[:framenum + 1, :]
            #nodes_current = nodes_abs[:framenum + 1, :]
