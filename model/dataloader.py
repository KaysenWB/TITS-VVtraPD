...
class Trajectory_Dataloader():
    def __init__(self, args):
        ...
        self.args = args
        # make test and train dataset arguments
        if self.args.dataset == 'eth5':
            ...
            # separate the train and test set here
    ...
    def traject_preprocess(self, setname):
        ...
        for seti, directory in enumerate(data_dirs):
            ...
            data = np.genfromtxt(os.path.join(directory, 'true_pos_.csv'), delimiter=',')
            Pedlist = np.unique(data[1, :]).tolist() # Get Ped IDs

            # create space for every datasets
            numFrame_data.append([])
            frameped_dict.append({})
            pedtrajec_dict.append({})

            for ind, pedi in enumerate(Pedlist):
                FrameContainPed = data[:, data[1, :] == pedi]  # get trajectories of every person
                FrameList = FrameContainPed[0, :].tolist()  # get existed frames for every person
                if len(FrameList) < 2:
                    continue             # remove too short trajectory
                numFrame_data[seti].append(len(FrameList)) # Add number of frames of this trajectory
    ...
    def get_data_index(self, data_dict, setname, ifshuffle=True):
        '''
        Get a ndarray [frames_id_in_set (concating the frame_ids of every datasets),
                       set_id           (0 ~ number of datasets),
                       full_frame_id    (0 ~ num of all frames of 7 dataset), ]
        '''
        set_id = []
        frame_id_in_set = []
        total_frame = 0
    ...
    def dataPreprocess(self, setname):
        ...
        if setname != 'train':
            ...
            shuffle = True
        data_index = self.get_data_index(frameped_dict, setname, ifshuffle=shuffle) # ids [fra_ids_in_every_dataset, datasets_id, total_fra_id ]
    ...
    def get_seq_from_index_balance(self, frameped_dict, pedtraject_dict, data_index, setname):
        ...
        for i in range(data_index.shape[1]):
            ...
            # get start and end frame id for a sample
            if i % 100 == 0:
            ...
            try:
                ...
                continue
            present_pedi = framestart_pedi | frameend_pedi  # union set

            if (framestart_pedi & frameend_pedi).__len__() == 0:   # intersection set
                continue

            traject = ()
            IFfull = []
            # process every trajectory in a sample
            for ped in present_pedi:
                ...
                if len(cur_trajec) == 0:
                    continue
                if ifexistobs == False:
                    continue # Just ignore trajectories if their data don't exsist in the observed time
                if sum(cur_trajec[:, 0] > 0) < 5:
                    continue # filter trajectories have too few frame data

                # from [time_step, 3] to [time_step, 2](remove frame) then to [time_step, 1, 2]
                cur_trajec = (cur_trajec[:, 1:].reshape(-1, 1, 2),)
                traject = traject.__add__(cur_trajec)
                IFfull.append(iffull)

            if traject.__len__() < 1:
                continue  # drop it, if the number of ped/tra in a sample is too small
            if sum(IFfull) < 1:
                continue  # drop it, if there is no a completed trajectory
            traject_batch = np.concatenate(traject, 1) # a sample [lens/timestep, nodes/ped, feats/x, y]
            batch_pednum = sum([i.shape[1] for i in batch_data]) + traject_batch.shape[1]
                           # peds in past samples
            cur_pednum = traject_batch.shape[1]  # peds in a sample
            ped_cnt += cur_pednum
    ...
    def find_trajectory_fragment(self, trajectory, startframe, seq_length, skip):
        ...
        # counting Steps from start_frame to fist point of trajectory
        offset_start = int((candidate_seq[0, 0] - startframe) // skip)
        # counting Steps from end_frame back to last point of trajectory
        ...
        if return_trajec[self.args.obs_length - 1, 1] != 0:
            ifexsitobs = True  # if obstacles existing

        if offset_end - offset_start >= seq_length - 1:
            iffull = True       # if full trajectory
    ...
    def get_social_inputs_numpy(self, inputnodes):
        ...
        # tell us where data exist (= 1) in [seq, nodes]
    ...
    def rotate_shift_batch(self, batch_data, ifrotate=True):
        ...
        # the meaning of 'batch - shift_value' is change coordinate origin to the observed point,  change coordinate.
